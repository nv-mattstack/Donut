...
Stock raster passes use a separate `input_assembler_float` vertex shader for FP32/FP16 inputs, avoiding decode constants and per-draw push updates. UNORM16 keeps its decode constants. `RenderView` can reuse unchanged UNORM constants between draws, invalidating them after every graphics-state change and at each recording scope boundary. Direct callers of the pass methods remain uncached and do not need cache-management notifications. Buffer-load rendering still supplies its full per-draw constants.

Derived passes retain their existing shader and binding behavior by default. A subclass can override `SupportsInputAssemblerTexCoordSpecialization()` to return `true` after confirming that it uses the stock vertex/input binding factories and that no other shader stage or custom draw logic consumes those push constants. This opt-in permits the floating-point shader replacement, shared input binding sets, and UNORM constant caching. Subclasses with custom vertex shaders or additional uses of the constants should retain the default behavior.
